...
from projects.mmdet3d_plugin.EAD.utils.traj_lr_warmup import get_traj_warmup_loss_weight
from projects.mmdet3d_plugin.EAD.utils.map_utils import (
...
class EADHead(DETRHead):
    ...
    def __init__(self,
        ...
        if map_class_weight is not None and (self.__class__ is EADHead):
        ...
        super(EADHead, self).__init__(*args, transformer=transformer, **kwargs)
        ...
        self.brain_key_pos = torch.nn.Parameter(torch.randn(1, 1, 256))
        self.brian_query_pos = torch.nn.Parameter(torch.randn(1, 1, 256))
        self.brain_key_mlp = torch.nn.Linear(self.eeg_dim, 256)
        #self.brain_value_mlp = torch.nn.Linear(self.eeg_dim, 256)
    ...
    def _init_layers(self):
        ...
        if self.eeg_decoder is not None:
            self.eeg_decoder = build_transformer_layer_sequence(self.eeg_decoder)

        ego_fut_decoder = []
        ego_fut_dec_in_dim = self.embed_dims*3 + len(self.ego_lcf_feat_idx) \
            if self.ego_lcf_feat_idx is not None else self.embed_dims*3
        for _ in range(self.num_reg_fcs):
        ...
        if self.ego_map_decoder is not None:
            for p in self.ego_map_decoder.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
        if self.eeg_decoder is not None:
            for p in self.eeg_decoder.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
    ...
    def forward(self,
        ...
        brain_q_with_pos = ego_map_query + self.brian_query_pos
        # brain_k_with_pos = brain_feats + self.brain_key_pos 
        brain_key = self.brain_key_mlp(brain_feats)
        brain_k_with_pos = brain_key + self.brain_key_pos 
        #brain_value = self.brain_value_mlp(brain_feats)
        # key_padding_mask = torch.zeros(1, 1, dtype=torch.bool).to('cuda')
        ego_eeg_query = self.eeg_decoder(
            query=brain_q_with_pos,
            key=brain_k_with_pos,
            value=brain_key,
            query_pos=None,
            key_pos=None)        
